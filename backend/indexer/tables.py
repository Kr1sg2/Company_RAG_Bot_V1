    ...
    def extract_tables_from_page(self, pdf_path: str, page_num: int) -> List[Dict]:
        ...
        try:
            ...
            lattice_tables = camelot.read_pdf(
                pdf_path, pages=str(page_num), flavor="lattice", line_scale=40
            )
        ...
        if not tables:
            try:
                stream_tables = camelot.read_pdf(
                    pdf_path, pages=str(page_num), flavor="stream", edge_tol=500
                )
    ...
    def tables_to_text(self, tables: List[Dict]) -> str:
        ...
        for table in tables:
            try:
                # Add table metadata
                text_parts.append(
                    f"TABLE {table['table_index']} (accuracy: {table['accuracy']:.1f}%):"
                )
