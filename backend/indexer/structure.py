...
def extract_pdf_outline(pdf_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(
            f"Extracted {len(outline)} PDF bookmarks from {os.path.basename(pdf_path)}"
        )
        return outline
            ...
            if (
                ...
                and len(
                    [
                        ...
                        for w in words
                        if w.lower()
                        in ["the", "of", "and", "to", "in", "for", "on", "with"]
                    ]
                )
                <= len(words) // 3
            ):
...
def build_heading_index(
    pdf_path: str, pages_text: Dict[int, str]
) -> List[Dict[str, Any]]:
    ...
    for heading in all_headings:
        ...
        for existing in deduplicated:
            if (
                existing["page"] == heading["page"]
                and _text_similarity(existing["title"].lower(), title_lower) > 0.8
            ):
    ...
    logger.info(
        f"Built heading index for {os.path.basename(pdf_path)}: "
...
def tag_chunk(
    chunk_text: str, page: int, chunk_index: int, heading_index: List[Dict[str, Any]]
) -> Dict[str, Any]:
...
def extract_structure_for_document(
    pdf_path: str, pages_text: Dict[int, str]
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(
            f"Structure extraction complete for {os.path.basename(pdf_path)}: "
