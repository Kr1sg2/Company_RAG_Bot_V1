...
class IndexerConfig:
    ...
    def __init__(self):
        ...
        extensions_str = os.getenv(
            "LEXA_EXTS", 
            ".pdf,.docx,.doc,.pptx,.ppt,.txt,.md,.rtf,.xlsx,.xls,.csv"
        )
        ...
        self.SMART_CHUNK_OVERLAP_TOKENS = int(os.getenv("SMART_CHUNK_OVERLAP_TOKENS", "80"))
        self.SMART_CHUNK_MERGE_NEARBY = os.getenv("SMART_CHUNK_MERGE_NEARBY", "1") == "1"
    ...
    def validate(self) -> list[str]:
        ...
        if not (0.0 <= self.CHUNK_OVERLAP <= 1.0):
            errors.append(f"CHUNK_OVERLAP must be between 0.0 and 1.0: {self.CHUNK_OVERLAP}")
