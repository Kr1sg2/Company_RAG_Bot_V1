...
# Detect PIL availability without importing at runtime
PIL_AVAILABLE = importlib.util.find_spec("PIL") is not None

# Only import for type checking to avoid runtime dependency
if TYPE_CHECKING:
...
class OCRProcessor:
    def __init__(self):
        self.available = all(
            [PDF2IMAGE_AVAILABLE, PIL_AVAILABLE, PYTESSERACT_AVAILABLE]
        )
        if not self.available:
            logger.warning(
                "OCR not available - missing dependencies (pdf2image, PIL, or pytesseract)"
            )
    ...
    def extract_from_bytes(
        self, pdf_bytes: bytes, page_num: int, dpi: int = 300
    ) -> Tuple[str, int]:
