...
def _infer_chunk_type(text, meta):
    ...
    if "q&a" in t or "faq" in t or t.startswith("q:") or "faq" in name or "faq" in hpth:
        return "faq"
    if (
        "procedure" in t
        or "step " in t
        or "steps:" in t
        or "how to" in t
        or "procedure" in hpth
    ):
        return "procedure"
    if "form" in t or "form" in name:
        return "form"
    return "passage"
...
try:
    ...
    for cls_name in ("DocumentStore", "Store"):
        ...
        if C:
            for m in ("add_documents", "upsert_documents"):
                _wrap_on(C, m, has_chunks=False)
            for m in ("add_chunks", "upsert_chunks"):
                _wrap_on(C, m, has_chunks=True)
except Exception:
    pass
...
try:
    ...
    _C = _cmod.Collection
    for m in ("add", "upsert"):
        _wrap_on(_C, m, has_chunks=False)
except Exception:
    pass
