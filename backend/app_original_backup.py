...
OCR_TRIGGER_MIN_WORDS = int(os.getenv("OCR_TRIGGER_MIN_WORDS", "0"))  # pages with fewer than this trigger OCR
...
def is_valid_chunk(chunk: str) -> bool:
    ...
    if len(chunk) < min_chunk_size:
        logger.debug(f"Chunk rejected: too short ({len(chunk)} < {min_chunk_size} chars)")
        return False
    
    printable_ratio = sum(c.isprintable() for c in chunk) / len(chunk)
    if printable_ratio <= 0.6:
        logger.debug(f"Chunk rejected: low printable ratio ({printable_ratio:.2f} <= 0.6)")
        return False
    ...
    for page_num, page in enumerate(doc, start=1):
        ...
        if OCR_ENABLED and native_word_count <= OCR_TRIGGER_MIN_WORDS:
            ...
            if pdf_path and ocr_is_available():
                try:
                    ocr_text = ocr_pdf_page(pdf_path, page_num, dpi=OCR_DPI, lang=OCR_LANG)
                    ocr_word_count = count_words(ocr_text)
                    logger.info(f"OCR via pdf2image: page {page_num}, native_words={native_word_count}, ocr_words={ocr_word_count}")
                except Exception as e:
            ...
            if not ocr_text and pytesseract and Image:
                try:
                    pix = page.get_pixmap(alpha=False, matrix=fitz.Matrix(OCR_DPI/72, OCR_DPI/72))
                    mode = "RGBA" if getattr(pix, "alpha", False) else "RGB"
                    img = Image.frombytes(mode, [pix.width, pix.height], pix.samples)
                    if mode == "RGBA":
                        img = img.convert("RGB")
                    ocr_text = pytesseract.image_to_string(img, lang=OCR_LANG, config="--oem 3 --psm 6")
                    ocr_word_count = count_words(ocr_text)
                    logger.info(f"OCR via PyMuPDF: page {page_num}, native_words={native_word_count}, ocr_words={ocr_word_count}")
                except Exception as e:
            ...
            if ocr_word_count > native_word_count:
                final_text = ocr_text
                logger.info(f"Using OCR text for page {page_num}: {ocr_word_count} words vs {native_word_count} native words")
            elif ocr_word_count > 0:
                # Merge texts if both have content
                final_text = f"{native_text}\n{ocr_text}".strip()
                logger.info(f"Merged OCR+native for page {page_num}: {ocr_word_count} OCR + {native_word_count} native words")
        ...
        if native_word_count < 20 and final_word_count >= native_word_count:
            logger.info(f"Page {page_num} with {native_word_count} native words kept in index (final: {final_word_count} words)")
...
def chat_with_openai(
    ...
    system_prompt = custom_system_prompt or system_prompt or ""  # system_prompt was initialized above
...
async def _req_logger(request, call_next):
    if request.url.path.startswith("/api/"):
        req_logger.info("REQ %s %s origin=%s", request.method, request.url.path, request.headers.get("origin"))
