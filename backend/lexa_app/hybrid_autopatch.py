...
def _tok(s: str): return _WORD.findall((s or "").lower())
...
_SYNONYMS = {
  ...
  "approval": ["authorize","sign off","sign-off","approve","permission","signoff"],
...
def _expand_queries(q: str, limit: int = 5):
    ...
    for m in re.findall(r'"([^"]+)"|(\w+\s+\w+)', q):
        ...
        if phrase and " " in phrase: ex.add(f'"{phrase}"')
    # synonym bumps
...
PRESENCE_BONUS={'approval':0.15,'limit':0.15,'pricing':0.15,'price':0.15,'eligible':0.10}
def _bm25(qtoks, dtoks, idf, avgdl, k1=1.2, b=0.75):
    ...
    dl = len(dtoks)
    for t in qtoks:
        if t not in idf or t not in freq: continue
        tf = freq[t]
