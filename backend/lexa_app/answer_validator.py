...
def is_enabled() -> bool:
    ...
    return os.getenv("LEXA_USE_ANSWER_VALIDATION", "false").lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def detect_completeness_issues(text: str) -> List[str]:
    ...
    looks_like_list = bool(
        re.search(r"^(\s*(\d+[\.)]|[-\*•]))\s+", stripped, re.MULTILINE)
    )
    ...
    # Vague references
    if re.search(
        r"\b(as mentioned|as described|see above|follow these|the process)\b",
        text,
        re.IGNORECASE,
    ):
    ...
    if re.search(r"\b(this|that|it|these|those)\b", text, re.IGNORECASE):
        context_ratio = len(
            re.findall(r"\b(this|that|it|these|those)\b", text, re.IGNORECASE)
        ) / len(text.split())
...
def validate_answer(response_text: str, sources: List[Dict], query: str = "") -> Dict:
    ...
    if (
        ...
        and any(
            word in query.lower() for word in ["explain", "describe", "comprehensive"]
        )
        if query
        else False
    ):
...
def enhance_answer_with_validation(
    response_text: str, sources: List[Dict], query: str = ""
) -> str:
    """
    ...
    if validation["step_count"] < 3 and any(
        keyword in query.lower() for keyword in ["how", "steps", "process"]
    ):
        enhancement_lines.append(
            "*This appears to be a multi-step process. You may need to check the full document for additional steps.*"
        )

    if validation["citation_count"] == 0:
        enhancement_lines.append(
            "*Consider checking the source documents directly for complete procedures.*"
        )

    if any("truncated" in issue for issue in validation["completeness_issues"]):
        enhancement_lines.append(
            "*This answer may be incomplete. Try asking for specific sections or breaking down your question.*"
        )
