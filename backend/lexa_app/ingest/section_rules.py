...
SUPPLY_HEADING_RX = re.compile(r'^\s*(?:supplies|materials|you will need|tools required)[:.]?\s*$', re.IGNORECASE)
...
PRODUCT_CARE_PATTERNS = {
    'keywords': ['quick fix', 'rattan', 'white wicker', 'blush spray', 'lemon oil', 
                'cleaning', 'maintenance', 'touch up', 'repair', 'scratch'],
    'section_headers': ['rattan', 'wicker', 'aluminum', 'fabric', 'cushions', 'frames']
}

NETSUITE_PATTERNS = {
    'keywords': ['login', 'navigate to', 'dashboard', 'field:', 'netsuite', 'sales order',
                'customer', 'inventory', 'billing', 'account', 'screen', 'tab'],
    'section_headers': ['procedure', 'steps', 'navigation', 'process']
}

POLICY_PATTERNS = {
    'keywords': ['policy', 'scope', 'effective date', 'procedure', 'guidelines',
                'employee', 'benefits', 'pto', 'vacation', 'discipline'],
    'section_headers': ['purpose', 'scope', 'policy', 'procedure', 'definitions']
}

FORM_PATTERNS = {
    'keywords': ['form', 'request', 'submit', 'application', 'fill out', 'complete',
...
def detect_document_category(text: str, doc_type: str = None) -> str:
    ...
    scores = {
        'product_care': sum(1 for kw in PRODUCT_CARE_PATTERNS['keywords'] if kw in text_lower),
        'netsuite_guide': sum(1 for kw in NETSUITE_PATTERNS['keywords'] if kw in text_lower),
...
def detect_section_headers(text: str) -> List[Tuple[str, int, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        elif len(line_stripped) < 80 and line_stripped.isupper() and len(line_stripped) > 5:
    ...
    for line in lines:
        ...
        if in_supply_section:
            if BULLET_RX.match(line_stripped) or line_stripped.startswith('-'):
                ...
                line_stripped == '' or
                ROMAN_NUMERAL_RX.match(line_stripped) or
                LETTER_SECTION_RX.match(line_stripped) or
                NUMBER_SECTION_RX.match(line_stripped) or
                SUBSECTION_RX.match(line_stripped) or
...
def detect_form_fields(text: str) -> Dict:
    ...
    form_indicators = sum(1 for pattern in FORM_PATTERNS['indicators'] if pattern in text)
    ...
    field_count = sum(len(re.findall(pattern, text, re.IGNORECASE)) for pattern in field_patterns)
