...
PROCESS_CHAINS = {
    'quote to sale': ['convert quote to sales order', 'quote conversion process', 'sales order from quote'],
    'opportunity to sale': ['convert opportunity', 'close opportunity', 'opportunity conversion'],
    'customer onboarding': ['create customer', 'customer setup', 'new customer process'],
...
def is_enabled() -> bool:
    ...
    return os.getenv("LEXA_USE_QUERY_REWRITE", "false").lower() in ("1", "true", "yes", "on")
...
def get_query_intent(query: str) -> str:
    ...
    if any(nav in query_lower for nav in [
        'where is', 'where do i find', 'locate', 'menu', 'button', 'page'
    ]):
        return 'navigation'
