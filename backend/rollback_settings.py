...
def rollback_from_backup(backup_path: Path, create_backup: bool = True) -> str:
    ...
    if create_backup and SETTINGS_FILE.exists():
        ...
        rollback_backup_msg = (
            f"📦 Created pre-rollback backup: {rollback_backup_name}\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Rollback Lexa AI settings from backup"
    )
    parser.add_argument(
        "--list", "-l", action="store_true", help="List all available backup files"
    )
    parser.add_argument(
        "--backup", "-b", type=str, help="Specific backup file to restore from"
    )
    parser.add_argument(
        "--latest", action="store_true", help="Restore from the most recent backup"
    )
    parser.add_argument(
        "--preview", "-p", type=str, help="Preview contents of a backup file"
    )
    parser.add_argument(
        "--no-backup",
        action="store_true",
    ...
    try:
        if args.list:
            ...
            for i, (backup_path, timestamp) in enumerate(backups):
                ...
                print(
                    f"   {i+1}. {backup_name} ({timestamp.strftime('%Y-%m-%d %H:%M:%S')}, {age.days} days ago)"
                )
            return

        if args.preview:
            backup_path = (
                Path(STORAGE_DIR) / args.preview
                if not args.preview.startswith("/")
        ...
        if args.latest:
            ...
            backup_path = (
                Path(STORAGE_DIR) / args.backup
                if not args.backup.startswith("/")
                else Path(args.backup)
            )
            if not backup_path.exists():
