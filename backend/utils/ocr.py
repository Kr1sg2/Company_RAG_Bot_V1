...
def ocr_pdf_page(pdf_path: str, page_number: int, dpi: int = 300, lang: str = "eng") -> str:
    """
    ...
    if not all([PDF2IMAGE_AVAILABLE, PIL_AVAILABLE, PYTESSERACT_AVAILABLE]):
        logger.warning("OCR dependencies not available (pdf2image, PIL, or pytesseract)")
        return ""
    ...
    try:
        ...
        logger.debug(f"OCR extracted {len(text)} characters from PDF page {page_number}")
        return text
