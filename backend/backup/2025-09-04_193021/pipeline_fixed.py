"""
Enhanced document processing pipeline with full environment flag support.
"""
...
from .chunk_fixed import chunk_processor
...
class EnhancedDocumentPipeline:
    ...
    def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            logger.debug(
                f"Generated {len(all_embeddings)} embeddings in {(len(texts) + batch_size - 1) // batch_size} batches"
            )
    ...
    def _is_image_only_page(self, text: str, word_count: int) -> bool:
        ...
        if word_count > 0:
            ...
            if (
                avg_word_length < 2
            ):  # Very short average word length suggests OCR artifacts
                return True
    ...
    def process_pdf_page(
        ...
        if not CONFIG.DISABLE_OCR and word_count < CONFIG.OCR_WORD_THRESHOLD:
            logger.info(
                f"Page {page_num} has {word_count} words, using OCR (OCR enabled: {not CONFIG.DISABLE_OCR})"
            )
            try:
                ...
                if ocr_word_count > word_count:
                    ...
                    logger.info(
                        f"OCR improved from {word_count} to {ocr_word_count} words"
                    )
            except Exception as e:
    ...
    def process_pdf(
        ...
        try:
            ...
            logger.info(
                f"Processing PDF {doc_id} with {total_pages} pages (OCR: {'disabled' if CONFIG.DISABLE_OCR else 'enabled'}, Tables: {'disabled' if CONFIG.DISABLE_TABLES else 'enabled'})"
            )
            ...
            for page_num, (page_text, tables) in enumerate(
                zip(page_texts, page_tables), 1
            ):
    ...
    def _detect_headers_footers(self, page_texts: List[str]) -> Dict[str, str]:
        ...
        if header or footer:
            logger.debug(
                f"Detected header: '{header[:50]}...', footer: '{footer[:50]}...'"
            )
    ...
    def process_document(self, file_path: str, watch_dir: str) -> bool:
        ...
        try:
            ...
            if ext not in CONFIG.ALLOWED_EXT:
                logger.debug(
                    f"Skipping {file_path}: extension {ext} not in allowed list"
                )
                return False
